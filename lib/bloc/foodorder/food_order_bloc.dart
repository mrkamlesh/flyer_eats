...
class FoodOrderBloc extends Bloc<FoodOrderEvent, FoodOrderState> {
  ...
  Stream<FoodOrderState> mapStartEditFoodDetailToState(FoodCartItem cartItem) async* {
    yield LoadingGetFoodDetail(placeOrder: state.placeOrder);
            ...
            if (result.addOnsTypes[i].id == cartAddOn.addOnsTypeId) {
              for (int j = 0; j < result.addOnsTypes[i].addOns.length; j++) {
                if (result.addOnsTypes[i].addOns[j].id == cartAddOn.id) {
                  result.addOnsTypes[i].addOns[j].isSelected = cartAddOn.isSelected;
  ...
  Stream<FoodOrderState> mapUpdateFoodDetailToState(
      FoodCartItem cartItem, int quantity, Price price, List<AddOn> addOns) async* {
    FoodCart foodCart = state.placeOrder.foodCart;

    int index = foodCart.multipleItemCart.indexOf(cartItem);

    foodCart.multipleItemCart[index] = FoodCartItem(cartItem.id, cartItem.food, quantity, price, addOns);

    yield FoodOrderState(placeOrder: state.placeOrder.copyWith(foodCart: foodCart));
