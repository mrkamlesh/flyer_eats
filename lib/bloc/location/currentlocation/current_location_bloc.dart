...
class CurrentLocationBloc extends Bloc<CurrentLocationEvent, CurrentLocationState> {
  ...
  Stream<CurrentLocationState> mapGetCurrentLocationToState() async* {
    yield LoadingCurrentLocationState(lat: state.lat, lng: state.lng, address: state.address);
    try {
      ...
      yield ErrorCurrentLocationState("Can not get current location", lng: state.lng, lat: state.lat, address: state.address);
    }
  }

  Stream<CurrentLocationState> mapUpdateAddressToState(double lat, double lng) async* {
    yield LoadingCurrentLocationState(lat: lat, lng: lng, address: state.address);
    try {
      ...
      yield ErrorCurrentLocationState("Can not get selected location", lng: lat, lat: lng, address: "...");
    }
  }
}
