...
class CancelledOrderPage extends StatefulWidget {
  ...
  const CancelledOrderPage({Key key, this.orderId, this.token, this.address, this.merchantId}) : super(key: key);
...
class _CancelledOrderPageState extends State<CancelledOrderPage> {
  ...
  void initState() {
    ...
    _bloc.add(GetSimilarRestaurant(widget.token, widget.address, widget.merchantId));
  }
                            ...
                            children: <Widget>[
                              BlocBuilder<CancelledOrderBloc, CancelledOrderState>(
                                ...
                                builder: (context, state) {
                                  if (state is LoadingSimilarRestaurant) {
                                    return Container(
                                      margin: EdgeInsets.only(
                                          bottom: distanceBetweenSection, top: distanceBetweenSection - 10),
                                      child: Center(
                                        child: SpinKitCircle(
                                  ...
                                  } else if (state is SuccessSimilarRestaurant) {
                                    return Column(
                                      children: <Widget>[
                                        Container(
                                          padding: EdgeInsets.symmetric(horizontal: horizontalPaddingDraggable),
                                          margin: EdgeInsets.only(
                                              bottom: distanceSectionContent - 10, top: distanceBetweenSection - 10),
                                          child: Row(
                              ...
                              BlocBuilder<CancelledOrderBloc, CancelledOrderState>(
                                condition: (context, state) {
                                  if (state is LoadingAds || state is SuccessAds || state is ErrorAds) {
                                    return true;
                                  } else {
                                    return false;
                                  }
