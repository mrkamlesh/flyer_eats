...
class ScratchCardListBloc extends Bloc<ScratchCardListEvent, ScratchCardListState> {
  ...
  Stream<ScratchCardListState> mapGetScratchCardListToSTate(String token) async* {
    yield LoadingList(scratchAmountTotal: state.scratchAmountTotal, scratchList: state.scratchList);
    try {
  ...
  Stream<ScratchCardListState> mapDoScratchCardToState(String token, String cardId, int pos) async* {
    try {
      ...
      if (result) {
        ...
        ScratchCard sc = state.scratchList[pos].copyWith(isScratched: true);
        List<ScratchCard> list = state.scratchList;
        list[pos] = sc;
