...
class DataRepository {
  ...
  Future<LoginStatus> checkPhoneExist(
    ...
    final response =
        await _provider.checkPhoneExist(contactPhone, otpSignature);
    if (response['code'] == 1) {
  ...
  Future<LoginStatus> loginByEmail(String contactPhone, String email,
      String password, String otpSignature) async {
    final response = await _provider.loginByEmail(
        contactPhone, email, password, otpSignature);
    if (response['code'] == 1) {
      return LoginStatus(response['msg'], true);
    } else {
      throw AppException("", response['msg']);
...
/*  Future<User> loginWithEmail(String email, String password) async {
  ...
  Future<List<Food>> getFoods(String restaurantId, String categoryId,
    ...
    final response = await _provider.getFoods(
        restaurantId, categoryId, isVegOnly, searchKeyword, address);
    if (response['code'] == 1) {
